...
Triage 会显式加载 [`config/triage_policy.yml`](./config/triage_policy.yml) 中的规则定义和对比样例，而不是靠模型猜边界。如果低风险 Triage 输出连续无法通过 Schema 校验，确定性回退会给出设备故障、连接、恢复、交易异常和未授权转账五类选项，不会把普通咨询误送人工。
