...
flowchart LR
    ...
    TR -->|明确资金安全事件| ES["escalated<br/>人工接管"]
...
高风险输出必须满足 Pydantic 约束。例如确认的钱包秘密泄露、钓鱼或未经授权资产转移必须是 `critical / P0 / escalate`，模型不能把它们静默降级。物理设备丢失、恢复后余额为零、普通交易失败和第三方固件知识咨询默认仍由 AI 说明或澄清，不能仅凭“丢失”“余额”“失败”等单个词升级。

Triage 会显式加载 [`config/triage_policy.yml`](./config/triage_policy.yml) 中的规则定义和对比样例，而不是靠模型猜边界。正常情况下，DeepSeek 根据当前问题动态生成 2–5 个上下文相关、互斥的结构化候选项：开机问题会生成开机现象，蓝牙问题会生成连接现象，不存在一套对所有问题通用的固定五项。只有低风险 Triage 输出连续无法通过 Schema 校验时，确定性回退才会给出设备故障、连接、恢复、交易异常和未授权转账五类兜底方向。

低风险问题采用渐进式澄清：像“我的钱包有问题”这种无法确定方向的输入，会先给出一个具体问题和 2–5 个动态选项；像“固件升级中断了怎么办”这种意图已明确但缺少设备型号、错误码的输入，会继续检索知识库，先返回经 Review 审核的安全恢复建议，再提示用户补充必要信息。每个动态选项由系统生成稳定 `choice_id` 并保存结构化语义；用户点击后，Runtime 只接收工单 ID 和 choice ID，再从 Repository 取回可信语义直接恢复路由，不会把短标签重新交给模型猜一次，因此不会循环询问同一个问题。自由文本回复仍会重新进入 Triage。
...
- **两阶段客户提交**：先持久化一个用户消息和处理中占位，再禁用客户命令控件并执行；重复 rerun 只检查原命令，不把 `in_progress` 误报成最终失败；
...
客户页始终展示“联系人工客服”入口。用户收到第一条 AI 回答后，只要不满意就可以立即申请人工，不设置三次交互门槛。系统只对明确的资金安全事件自动升级：确认的钱包秘密泄露、钓鱼、未经授权资产转移、地址不一致和可疑签名；普通设备、恢复、交易 pending/失败与保修知识问题继续由 AI 回答。
...
设备重置、bootloader 恢复和保修结论等有影响的动作不会被 Tool 自动执行。AI 可以给准备事项、风险提醒和非执行性说明，但不能声称已经重置设备或批准保修；用户需要真实业务操作时，可以主动点击人工入口。
...
| 保修判断 | 提供演示序列号后四位 `A1B2` | 查询模拟证据并给出非承诺性说明；用户可自主申请人工处理 |
| 未授权转账 | “我发现一笔不是自己发起的转账” | 识别 `asset_loss`，自动进入资金安全人工队列 |
...
• 实现持久化前敏感信息脱敏、风险粘性和出站 Policy Guard，覆盖助记词、私钥、
  PIN、Passphrase 与不可信链接等安全边界；用户首轮即可主动转人工，明确资金安全事件自动升级。
